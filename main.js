...
// Data limite: 20 de Dezembro de 2025 (Mês 11 = Dezembro)
...
// 1. Mostrar/Esconder se vai ou não
if(selectStatus){
...
// 2. Gerar campos para ADULTOS EXTRAS
if(selectAdultos){
...
function checarPrazo() {
    const hoje = new Date();
    // Verifica se tem ?lote=2 na URL para ignorar prazo
...
if(form){
    form.addEventListener('submit', function(e) {
        ...
        // Coleta Nomes Adultos
        let nomesAdultosExtras = "Nenhum";
        const inputsAdultos = document.querySelectorAll('.input-adulto-extra');
        if(inputsAdultos.length > 0) {
            nomesAdultosExtras = Array.from(inputsAdultos).map(input => input.value).join(', ');
        }

        // Coleta Crianças (AQUI ESTAVA O ERRO)
